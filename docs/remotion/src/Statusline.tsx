...
export const Statusline: React.FC = () => {
  ...
  //   150-210 Phase 4: rate-limit bars fill 0 -> 28% (current) and 0 -> 14% (weekly)
  //   210-240 Phase 5: hold final state
  ...
  const brainPulse = interpolate(
    frame,
    [122, 140, 158, 176, 194, 212, 230],
    [1, 1.12, 1, 1.12, 1, 1.12, 1],
    { extrapolateLeft: "clamp", extrapolateRight: "clamp" },
  );

  // Phase 4 — rate limits.
  const rateLabelAppear = appearAt(frame, 150);
  const fivePct = interpolate(frame, [150, 210], [0, 28], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.out(Easing.ease),
  });
  const sevenPct = interpolate(frame, [165, 215], [0, 14], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.out(Easing.ease),
  });
  const projectionAppear = appearAt(frame, 196);
